...
<#assign download_archive_url>${baseURL}/archive.do?r=${resource.shortname}<#if version??>&v=${version.toPlainString()}</#if></#assign>
...
<script>
    $(document).ready(function() {
        ...
        const fields = [
            { value: '${eml.description!?json_string}',      id: 'description-container' },
            { value: '${eml.introduction!?json_string}',     id: 'introduction-container' },
            { value: '${eml.purpose!?json_string}',          id: 'purpose-container' },
            { value: '${eml.acknowledgements!?json_string}', id: 'acknowledgements-container' },
            { value: '${eml.gettingStarted!?json_string}',   id: 'gettingStarted-container' }
        ];

        // Render content FIRST, before anything measures the page.
                    ...
                    <div class="text-gbif-primary fs-smaller-2 mt-2">
                        <span>
                            ...
                            <#if resource.organisation??>
                                <#if defaultOrganisation?? && resource.organisation.key.toString() == defaultOrganisation.key.toString()>
                            ...
                            <#else>
                                <@s.text name='portal.resource.publishedOn.no.organisation'></@s.text> <span property="dc:issued">${(eml.pubDate)!?date?string("MMM d, yyyy")}</span>
                            </#if>
                        </span>
                    </div>
                <#else>
                                ...
                                <#if !metadataOnly>
                                    <tr>
                                        ...
                                        <#if version?? && version.toPlainString() != resource.emlVersion.toPlainString() && recordsPublishedForVersion??>
                                            <td class="p-0">
                                                <a href="${download_archive_url}" onClick="_gaq.push(['_trackEvent', 'Archive', 'Download', '${resource.shortname}', ${recordsPublishedForVersion!0?c} ]);">
                                                ...
                                                <#if !isDataPackage>
                                                    ${recordsPublishedForVersion!0?c}
                                                    <@s.text name='portal.resource.records'/>&nbsp;</#if>
                                                <#if eml.language?has_content && languages[eml.language]?has_content><@s.text name='eml.language.available'>
                                                    <@s.param>${languages[eml.language]?cap_first!}</@s.param></@s.text></#if>
                                                (${archiveSizeForVersion!})
                                                <#if eml.updateFrequency?has_content && eml.updateFrequency.identifier?has_content && frequencies[eml.updateFrequency.identifier]?has_content>&nbsp;-&nbsp;${updateFrequencyTitle?lower_case?cap_first}:&nbsp;${frequencies[eml.updateFrequency.identifier]?lower_case}
                                                </#if>
                                            </td>
                                        <#else>
                                            <td class="p-0">
                                                <a href="${download_archive_url}" onClick="_gaq.push(['_trackEvent', 'Archive', 'Download', '${resource.shortname}', ${resource.recordsPublished!0?c} ]);">
                                                    <svg class="link-icon" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="DownloadIcon"><path d="M5 20h14v-2H5v2zM19 9h-4V3H9v6H5l7 7 7-7z"></path></svg>
                                                    <@s.text name='portal.resource.download'/>
                                                </a>
                                                <#if !isDataPackage>${resource.recordsPublished!0?c} <@s.text name='portal.resource.records'/>&nbsp;</#if><#if eml.language?has_content && languages[eml.language]?has_content><@s.text name='eml.language.available'><@s.param>${languages[eml.language]?cap_first!}</@s.param></@s.text></#if> (${archiveSizeForVersion!})<#if eml.updateFrequency?has_content && eml.updateFrequency.identifier?has_content && frequencies[eml.updateFrequency.identifier]?has_content>&nbsp;-&nbsp;${updateFrequencyTitle?lower_case?cap_first}:&nbsp;${frequencies[eml.updateFrequency.identifier]?lower_case}</#if>
                                            </td>
                                        </#if>
                                    </tr>
                                </#if>
