...
/** A very simple utility class to encapsulate the basic layout of the data directory and to configure & persist the path for that directory
 * and make it available to the entire application.
 * @author markus
 *
 */
@Singleton
public class DataDir {
	...
	private final File dataDirSettingFile;
	private Log log = LogFactory.getLog(this.getClass());
	private InputStreamUtils streamUtils=new InputStreamUtils();

	
	public DataDir(File dataDirSettingFile) {
		super();
		this.dataDirSettingFile=dataDirSettingFile;
		if (dataDirSettingFile!=null && dataDirSettingFile.exists()){
			// a datadir has been configured already. Lets see where that is
			String dataDirPath=null;
			try {
				dataDirPath = StringUtils.trimToNull(org.apache.commons.io.FileUtils.readFileToString(dataDirSettingFile));
				if (dataDirPath!=null){
					log.info("IPT Data Directory configured at "+dataDirPath);
					dataDir=new File(dataDirPath);
				}
			} catch (IOException e) {
				log.error("Failed to read the datadir location settings file in WEB-INF at "+dataDirSettingFile.getAbsolutePath(), e);
			}				
		}else{
	...
	/** Sets the path to the data directory for the entire application and persists it in the /WEB-INF folder.
	 * This method does not reload any configuration though - so normally setting the dataDir should be done 
	 * through the ConfigManager which calls this method but also reloads all user configurations.
	 * @See ConfigManager
	 * @param dataDir
			...
			if (dataDir.exists() && !dataDir.isDirectory()){
				...
				try {
					...
					if (configDir.exists() && configDir.isDirectory()){
						...
						org.gbif.ipt.utils.FileUtils.copyStreamToFile(streamUtils.classpathStream("/configDefault/ipt.properties"), configFile(AppConfig.DATADIR_PROPFILE));
						org.gbif.ipt.utils.FileUtils.copyStreamToFile(streamUtils.classpathStream("/configDefault/about.ftl"), configFile("about.ftl"));
						org.gbif.ipt.utils.FileUtils.copyStreamToFile(streamUtils.classpathStream("/configDefault/users.xml"), configFile(UserAccountManagerImpl.PERSISTENCE_FILE));
					}
				} catch (IOException e) {
