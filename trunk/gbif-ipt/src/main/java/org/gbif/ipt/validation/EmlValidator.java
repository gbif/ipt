		...
		if (eml != null) {
			if (part == null || part.equalsIgnoreCase("basic")) {
				...
				for (int index = 0; index < eml.getAssociatedParties().size(); index++) {
					 ...
					 */
					if (exists(eml.getAssociatedParties().get(index).getAddress().getAddress()) || exists(eml.getAssociatedParties().get(index).getAddress().getCity())
							|| exists(eml.getAssociatedParties().get(index).getAddress().getProvince()) || exists(eml.getAssociatedParties().get(index).getAddress().getPostalCode())
						...
						if (!exists(eml.getAssociatedParties().get(index).getAddress().getCity())) {
							action.addFieldError("eml.associatedParties[" + index + "].address.city", action.getText("validation.required", new String[] { action
									.getText("eml.associatedParties.address.city") }));
						}
						if (!exists(eml.getAssociatedParties().get(index).getAddress().getProvince())) {
							action.addFieldError("eml.associatedParties[" + index + "].address.province", action.getText("validation.required", new String[] { action
									.getText("eml.associatedParties.address.province") }));
						}
						/* postal code should be a number */
						if (!exists(eml.getAssociatedParties().get(index).getAddress().getPostalCode())) {
							action.addFieldError("eml.associatedParties[" + index + "].address.postalCode", action.getText("validation.required", new String[] { action
									.getText("eml.associatedParties.address.postalCode") }));
						} else {
							try {
								Integer.parseInt(eml.getAssociatedParties().get(index).getAddress().getPostalCode());
							} catch (NumberFormatException e) {
								action.addFieldError("eml.associatedParties[" + index + "].address.postalCode", action.getText("validation.invalid", new String[] { action
										.getText("eml.associatedParties.address.postalCode") }));
							}
						}
						if (!exists(eml.getAssociatedParties().get(index).getAddress().getCountry())) {
							action.addFieldError("eml.associatedParties[" + index + "].address.country", action.getText("validation.required", new String[] { action
									.getText("eml.associatedParties.address.country") }));
						}
					}
