...
/**
 * The base of all IPT actions
 * This handles conditions such as menu items, a custom text provider, sessions, currently logged in user
 * @author tim
 */
public class BaseAction extends ActionSupport implements Action, SessionAware{
	...
	/**
	 * Utility to compare 2 objects for comparison when both converted to strings
	 * useful to compare if a submitted value is the same as the persisted value
  ...
  public boolean isAdminRights() {
	    ...
	    if(user != null && user.hasAdminRights()) return true;
	    return false;
	  }
  public boolean isManagerRights() {
	    User user = getCurrentUser();
	    if(user != null && user.hasManagerRights()) return true;
	    return false;
	  }
