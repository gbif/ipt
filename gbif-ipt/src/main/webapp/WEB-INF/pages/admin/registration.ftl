...
<#else>
	...
	<@s.form cssClass="topForm half" action="registration" method="post">
		<@selectList name="organisation.key" options="organisations" objValue="key" objTitle="name" keyBase="admin." value="" size=15 disabled=true/>  
		<@input name="organisation.password" keyBase="admin." type="text" disabled=true/>
		<@input name="organisation.alias" keyBase="admin." type="text" disabled=true/>
		<@checkbox name="organisation.canHost" keyBase="admin." disabled=true/>	
		<@s.hidden id="organisation.name" name="organisation.name" required="true" />
	   <div class="buttons">
	 	<@s.submit cssClass="button" name="save" id="save" key="button.save" disabled=true/>
	 	<@s.submit cssClass="button" name="cancel" id="cancel" key="button.cancel" disabled=true/>
	  </div>	  
	</@s.form>
</#if>
