 ...
 * @param <R>
 */
public abstract class ImportTask<R extends DataResource> extends
    TaskBase<UploadEvent, R> {
  ...
  static {
    List<RegionType> regionTypes = new ArrayList<RegionType>(
        RegionType.DARWIN_CORE_REGIONS);
  ...
  private final CacheMap<String, Taxon> taxonCache = new CacheMap<String, Taxon>(
      2500);
  private final LinkedList<Taxon> newTaxa = new LinkedList<Taxon>();
  private final CacheMap<String, Region> regionCache = new CacheMap<String, Region>(
      1000);
  private final LinkedList<Region> newRegions = new LinkedList<Region>();
  @Autowired
  ...
  public final UploadEvent call() throws Exception {
    ...
    try {
      ...
      // prepare import, removing previous records, calling prepare of helper
      // tasks
      prepare();
      Date now = new Date();
      log.info(String.format("Import preparation took %s ms",
          (now.getTime() - lastLogDate.getTime())));
      lastLogDate = now;
      ...
      for (ExtensionMapping vm : resource.getExtensionMappings()) {
        ...
        log.info(String.format(
            "Importing %s records of extension %s took %s ms",
            vm.getRecTotal(), vm.getExtension().getName(),
            (now.getTime() - lastLogDate.getTime())));
        lastLogDate = now;
      }
      ...
      close();
      now = new Date();
      log.info(String.format("Import closing took %s ms",
          (now.getTime() - lastLogDate.getTime())));
      lastLogDate = now;

      String msg = String.format("Import closing took %s ms",
          (now.getTime() - lastLogDate.getTime()));
      log.info(msg);
      annotationManager.annotateResource(resource, msg);

    } catch (InterruptedException e) {
      // thread was interrupted. Try to exit nicely by removing all potentially
      // corrupt data.
      prepare();
      annotationManager.annotateResource(resource,
          "Upload task was interrupted/canceled. Partially existing import was removed");
      throw e;
    }
  ...
  public final synchronized String status() {
    ...
    if (currentProcessed.get() > 0) {
      ...
      recordStatus = String.format(". %s records processed",
          currentProcessed.get());
    }
    ...
    if (currentProcessed != null) {
      return String.format("%s%s%s%s", currentActivity, recordStatus,
          subclassInfo, coreInfo);
    } else {
  ...
  /**
   * Extract unique Taxa from Darwin Core record, either the terminal taxon, but
   * potentially also the higher taxonomy. If dwc:HigherTaxon or
   * dwc:HigherTaxonID are mapped, the explicit higher taxonomy is *not*
   * extracted. Otherwise Each higher taxon becomes a new taxon if not already
   * existing.
   * 
   * @param dwc
   * @return
  ...
  protected Taxon extractTaxon(DarwinCore dwc) {
    ...
    if (terminalTaxon == null) {
      ...
      if (extractHigherTaxonomy) {
        // extract higher taxonomy if not already extracted and link terminal
        // taxon into hierarchy
        // try to find lowest persistent higher taxon
        // create new higher taxa as we go up and havent found a persistent one
        // yet
        Taxon parent = null;
        newTaxa.clear();
        boolean persistentParentFound = false;
        // first see if infraspecific epitheton exists.
        // This means there also is a species which we will use as a higher
        // taxon too
        if (dwc.getInfraspecificEpithet() != null) {
          ...
          if (parent == null) {
            ...
            // we cant assign a parent yet, therefor put it on the new taxon
            // stack
  ...
  /**
   * Hook for doing final processing for the entire resource, e.g. setting
   * statistics gathered via the record hook
   * 
   * @param resource
   */
  abstract void closeHandler(R resource);

  abstract void extensionRecordHandler(ExtensionRecord extRec);

  /**
   * Hook for doing initial preperations before processing the resource, e.g.
   * clearing statistics or removing old files
   * 
   * @param resource
   */
  abstract void prepareHandler(R resource);
   ...
   * @param record
   */
  abstract void recordHandler(DarwinCore record);
  ...
  private void close() {
    ...
    log.info(String.format(
        "Core import of %s records to cache done. %s deleted, %s added and %s records changed. %s bad records were skipped.",
        event.getRecordsUploaded(), event.getRecordsDeleted(),
        event.getRecordsAdded(), event.getRecordsChanged(),
        event.getRecordsErroneous()));
    ...
    event.setRecordsDeleted(resource.getRecTotal() + recordsAdded
        - recordsUploaded);
    event.setRecordsUploaded(recordsUploaded);
    event.setRecordsErroneous(recordsErroneous);
    event.setDuration();
    annotationManager.annotateResource(resource, "Complete import took "
    ...
    try {
      ...
      annotationManager.annotateResource(resource,
          "Could not write DarwinCore archive. IOException");
    } catch (Exception e) {
      log.error("Could not create DarwinCore archive", e);
      annotationManager.annotateResource(resource,
          "Could not create DarwinCore archive");
    }
  ...
  private Region findPersistentRegion(String mpath) {
    if (regionCache.containsKey(mpath)) {
      ...
      // cache is limited, so we need to check the db too to make sure it doesnt
      // exist
  ...
  private File importCore() throws InterruptedException {
    ...
    File out = null;
    try {
      // prepare core import source. Can be a file or database source to iterate
      ...
      try {
        ...
        for (ImportRecord irec : source) {
          // keep all annotations for a single record til the end so we can add
          ...
          try {
            ...
            // check if sourceID was unique. All old records should have deleted
            // flag=true
        ...
        // store final numbers in normal Integer so that the AtomicNumber can be
  ...
  private File importExtension(ExtensionMapping vm) throws InterruptedException {
    ...
    log.info(String.format("Starting import of %s extension for resource %s",
        extensionName, getTitle()));
    File out = null;
    // keep track of records for each extension and then store the totals in the
    // viewMapping.
    // once extension is imported this counter will be reset by the next
    // extension.
    // used to feed status()
    ...
    try {
      ...
      // catch any errors after we opened the source to close it properly and
      // set the extension statistics at least
      try {
        // Do we need a
        for (ImportRecord rec : source) {
          ...
          if (rec == null || rec.getSourceId() == null) {
            currentErroneous.addAndGet(1);
            annotationManager.badExtensionRecord(resource, extension, null,
                "Seems to be an empty record or missing source ID. Line "
                    + String.valueOf(currentProcessed.get()));
            continue;
          }
          ...
          try {
            ...
            annotationManager.badExtensionRecord(resource, extension,
                rec.getSourceId(), "Unkown error: " + e.toString());
          }
        ...
        annotationManager.annotateResource(resource, String.format(
            "Unknown error importing extension %s. Extension skipped",
            extensionName));
      ...
      annotationManager.annotateResource(resource, String.format(
          "Couldn't open import source for extension %s. Extension skipped",
          extensionName));
  ...
  private DarwinCore persistRecord(DarwinCore record, DarwinCore oldRecord) {
    ...
    if (oldRecord != null && oldRecord.hashCode() == record.hashCode()) {
      ...
      // also assign to old record cause this one gets saved if the record stays
      // the same. And the old taxon or region has been deleted already...
      ...
      // remove old record and save new one, preserving its GUID (copied in
  ...
  private void prepare() {
    ...
    // call subclass handler first, might need to remove dependend records
    // before removing the core records
    ...
    if (resource.getCoreMapping().hasMappedProperty("HigherTaxonID")
        || resource.getCoreMapping().hasMappedProperty("HigherTaxon")) {
