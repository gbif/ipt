...
+ [[Quality checks| dataQualityChecklist#quality-checks]]
  ...
  + [Who acted in the event?](dataQualityChecklist#who-acted-in-the-event)
    + [Case 1: Two different people collecting and identifying a specimen](dataQualityChecklist#case-1-two-different-people-collecting-and-identifying-a-specimen) 
  + [When did the event take place?](dataQualityChecklist#when-did-the-event-take-place)
    + [Case 1: Single date](dataQualityChecklist#case-1-single-date)
    + [Case 2: Date-time range spanning days](dataQualityChecklist#case-2-date-time-range-spanning-days)
    + [Case 3: Partial date](dataQualityChecklist#case-3-partial-date)
    + [Case 4: Missing date](dataQualityChecklist#case-4-missing-date)
  + [Where did the event take place?](dataQualityChecklist#where-did-the-event-take-place)
    + [Case 1: Point location converted from degrees minutes seconds to decimal degrees](dataQualityChecklist#case-1-point-location-converted-from-degrees-minutes-seconds-to-decimal-degrees)
    + [Case 2: Point location that was generalized](dataQualityChecklist#case-2-point-location-that-was-generalized)
    + [Case 3: Point location exists but not provided](dataQualityChecklist#case-3-point-location-exists-but-not-provided)
    + [Case 4: Point location does not exist](dataQualityChecklist#case-4-point-location-does-not-exist)
  + [Why did the event happen?](dataQualityChecklist#why-did-the-event-happen)
    + [Case 1: Because of a butterfly monitoring scheme](dataQualityChecklist#case-1-because-of-a-butterfly-monitoring-scheme)
...
| `Sampling Methods` | information about the sampling methodology used in creating the dataset, similar to the methods section of a journal article. _Note this is required for sampling event datasets._ | _See [here](http://ipt.ala.org.au/resource?r=global#methods)_ |
| `Sampling Methods` | how the dataset should be cited. Use of the [IPT Citation Format](https://github.com/gbif/ipt/wiki/IPT2Citation.wiki) (based on DataCite's preferred citation format and that satisfies the [Joint Declaration of Data Citation Principles](https://www.force11.org/datacitation)) is highly recommended. | _"Edgar G J, Stuart-Smith R D (2014): Reef Life Survey: Global reef fish dataset. v2.0. Reef Life Survey. Dataset/Samplingevent. http://doi.org/10.15468/qjgwba"_ |