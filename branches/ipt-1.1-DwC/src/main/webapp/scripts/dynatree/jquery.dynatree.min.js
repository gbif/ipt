// jquery.dynatree.js build 0.4.0_beta-2
// Revision: 172, date: 2009-02-08 22:56:13
...
tree.selectedNodes.push(this);},toString:function(){return"dtnode<"+this.data.key+">: '"+this.data.title+"'";},toDict:function(recursive){var dict=$.extend({},this.data);dict.activate=(this.tree.activeNode===this);dict.focus=(this.tree.focusNode===this);dict.expand=this.bExpanded;dict.select=this.bSelected;if(recursive&&this.childList){dict.children=[];for(var i=0;i<this.childList.length;i++)
...
this.span.innerHTML=this._getInnerHtml();this.div.style.display=(this.parent==null||this.parent.bExpanded?"":"none");var cn=this.tree.options.classNames;var cnList=[];cnList.push((this.data.isFolder)?cn.folder:cn.document);if(this.bExpanded)
cnList.push(cn.expanded);if(this.data.isLazy&&!this.isRead)
cnList.push(cn.lazy);if(this.isLastSibling())
...
if(bExpand&&this.data.isLazy&&!this.isRead){try{logMsg("_expand: start lazy - %o",this);this.setLazyNodeStatus(DTNodeStatus_Loading);if(true==opts.onLazyRead.call(this.span,this)){this.setLazyNodeStatus(DTNodeStatus_Ok);logMsg("_expand: lazy succeeded - %o",this);}}catch(e){this.setLazyNodeStatus(DTNodeStatus_Error);}
return;}
logMsg("_expand: start div toggle - %o",this);if(opts.fx){var duration=opts.fx.duration||200;$(">DIV",this.div).animate(opts.fx,duration);}else{var $d=$(">DIV",this.div);logMsg("_expand: got div, start toggle - %o",this);$d.toggle();}
logMsg("_expand: end div toggle - %o",this);},toggleExpand:function(){if(!this.childList&&!this.data.isLazy)
...
childNode=parentTreeNode._addNode(data);var $ul=$li.find(">ul:first");if($ul.length){self._createFromTag(childNode,$ul);}});},lastentry:undefined};$.widget("ui.dynatree",{init:function(){return this._init();},_init:function(){logMsg("Dynatree._init(): version='%s'.",DynaTree.version);this.options.event+=".dynatree";var $this=this.element;var opts=this.options;if(!opts.imagePath){$("script").each(function(){if(this.src.search(/.*dynatree[^/]*\.js$/i)>=0){if(this.src.indexOf("/")>=0)
...
$(divContainer).empty();this.tree=new DynaTree(divContainer,opts);var root=this.tree.getRoot();if(opts.children){root.append(opts.children);}else if(opts.initAjax&&opts.initAjax.url){root.appendAjax(opts.initAjax);}else if(opts.initId){this.tree._createFromTag(root,$("#"+opts.initId));}else{var $ul=$this.find(">ul").hide();this.tree._createFromTag(root,$ul);$ul.remove();}
this.bind();this.tree.initMode="postInit";nodeList=this.tree.selectedNodes.slice();this.tree.selectedNodes=[];for(var i=0;i<nodeList.length;i++){var dtnode=nodeList[i];logMsg("Re-select on init: %o",dtnode);dtnode.bSelected=false;dtnode.select(true);}
if(this.tree.focusNode){logMsg("Focus on init: %o",this.tree.focusNode);this.tree.focusNode.focus();}
