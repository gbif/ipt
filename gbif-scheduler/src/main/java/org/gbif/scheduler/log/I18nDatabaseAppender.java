...
import com.ibiodiversity.harvest.model.BioDatasource;
...
public class I18nDatabaseAppender extends AppenderSkeleton {
	...
	public static final String MDC_BIO_DATASOURCE = "mdc.bio_datasource";
	...
	protected void append(LoggingEvent event) {
		if (event.getMessage() instanceof I18nMessage
				&& I18nDatabaseAppender.logEventDao != null) {
			I18nMessage i18nMessage = (I18nMessage) event.getMessage();
			int level = I18nDatabaseAppender.getLevel(event.getLevel());
			
			BioDatasource bioDatasource = null;
			String instanceId = null;
			Long groupId = null;
			User user = null;
			
			// we are really typesafe... no exceptions here please!
			if (event.getMDC(MDC_BIO_DATASOURCE) instanceof BioDatasource) {
				bioDatasource = (BioDatasource) event.getMDC(MDC_BIO_DATASOURCE);
			}
			...
			if (infoAsJSON == null) {
				LogEvent logEvent = new LogEvent(bioDatasource,instanceId,groupId,
