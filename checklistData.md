...
**A.** First, you must publish your checklist and make it publicly available online under a GBIF-supported license (CC0, CC-BY, CC-BY-NC). GBIF can then manually review it to determine if it is a suitable backbone source, e.g. by looking at how its names overlap with the backbone. Ideally the checklist will use minimal classification, and be of high data quality meaning it has few name usage issues, include [scientificNameAuthorship](http://rs.tdwg.org/dwc/terms/#scientificNameAuthorship) of names, supplying the [namePublishedIn](http://rs.tdwg.org/dwc/terms/#namePublishedIn) reference, etc 