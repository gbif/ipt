...
|===
| Versión | Descripción                  | Date of release | Author(s)

| http://links.gbif.org/gbif_dwc-a_how_to_guide_en_v1[1.0]    | Content review and additions | abril 2011     | David Remsen, Markus Döring
| 2.0     | Transferred to wiki, major edits | 9 de majo de 2017      | Kyle Braak
| 2.1     | Minor edits, updating links etc  | 8 de febrero de 2021 | Matthew Blissett
| 2.2     | Conversion to AsciiDoc  | 25 de majo de 2021 | Matthew Blissett
|===
...
. *A required core data file* consisting of a standard set of http://rs.tdwg.org/dwc/terms/[Darwin Core terms]. El archivo de datos tiene el formato *texto en campo*, donde los registros de datos se expresan como filas de texto y los elementos de datos (columnas) se separan con un delimitador estándar, como una pestaña o una coma (comúnmente denominada CSV o 'https://es.wikipedia.org/wiki/Valores_separados_por_comas[valores separados por comas' archivos]).  The first row of the data file may optionally contain data or represent a ‘header row’. In general, if a header row is included, it contains the names of the Darwin Core terms represented in the succeeding rows of data.
...
<<Publicación de DwC-A utilizando la IPT>> is most suitable when:
