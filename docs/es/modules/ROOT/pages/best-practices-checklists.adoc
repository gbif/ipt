...
Los nombres vernáculos se referencian mediante una extensión, por lo que deben estar vinculados a un taxón con nombre en el archivo de datos principal. Se recomienda además que un registro de nombres vernáculos proporcione una referencia lingüística que identifique el idioma representado por el uso del nombre vernáculo. La mejor práctica es usar http://rs.gbif.org/vocabulary/iso/639-1.xml[código de idioma ISO 639] para compartir información lingüística. Los nombres vernáculos también pueden tener usos regionales distintos, lo que se puede especificar mediante un elemento dwc:locality o, a un nivel menos preciso, usando un término dwc:country. Se recomienda que los nombres de países utilicen http://rs.gbif.org/vocabulary/iso/3166-1_alpha2.xml[códigos de país ISO 3166-1].
...
Si se utiliza el elemento dwc:country, se recomienda utilizar los códigos de país ISO 3166-1 que aparecen en http://rs.gbif.org/vocabulary/iso/3166-1_alpha2.xml.
...
Tenga en cuenta que las extensiones deben vincularse a los taxones en el archivo de datos principal proporcionando el "taxonID" en el campo "taxonID". Consulte la <<checklist-data,structure of Darwin Core Archives>> arriba.
...
=== Extensión de Medición o Datos (Measurement or Facts) 

Última versión emitida: {latest-measurement-or-facts}[{date-measurement-or-facts}]   

Utilice esta extensión para proporcionar mediciones cuantitativas o datos cualitativos asociados a un taxón. Se pueden vincular varias mediciones o datos al mismo taxón mediante el identificador taxonID.

=== Extensión del Perfil de Especie(Species Profile) 

Última versión emitida: {latest-species-profile}[{date-species-profile}]

Utilice esta extensión para proporcionar características adicionales a la descripción textual que cubre la extensión de descripción. Normalmente, se trata de declaraciones booleanas (VERDADERO/FALSO) y respuestas breves para el resto. Para taxones con distintas combinaciones de valores (por ejemplo, distintos hábitats para diferentes etapas de vida), se pueden vincular varias características al mismo taxón mediante el identificador de taxón (taxonID).

=== Extensión Multimedia Sencilla (Simple Multimedia) 

Última versión emitida: {latest-simple-multimedia}[{date-simple-multimedia}]

Utilice esta extensión para compartir objetos multimedia de un taxón y documentar sus metadatos. Permite compartir un enlace al archivo multimedia mediante un identificador. Se pueden vincular varios archivos multimedia al mismo taxón a través del taxonID.

=== Extensión de Descripción de Medios Audiovisuales (Audiovisual Media Description)  

Última versión emitida: {latest-audiovisual-description}[{date-audiovisual-description}]

Utilice esta extensión para contenido multimedia que requiera una descripción exhaustiva, más allá de la que ofrece la extensión Simple Multimedia. Se pueden vincular varios archivos multimedia al mismo taxón mediante el identificador de taxón (taxonID).
