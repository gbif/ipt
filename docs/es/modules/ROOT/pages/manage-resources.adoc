...
If SMTP is configured in *IPT Settings → Email Notifications*, email notifications can be enabled by selecting "Email the IPT administrator if auto-publication fails".
...
.. Camtrap DP resources do not support alternative identifiers. Instead, add a new related identifier in the *Other Metadata* section, as shown below:
+
image::ipt2/manage/IPTCamtrapRelatedIdentifier.png[]
+
Be sure to use the correct link to the GBIF portal as a related identifier.
