...
--
* *Notificaciones por correo electrónico*: configure los ajustes SMTP para habilitar las notificaciones por correo electrónico del IPT. Estas notificaciones se utilizan para informar al administrador del IPT si falla la publicación automática.
+
Las siguientes opciones de configuración SMTP están disponibles:

** *Servidor SMTP* - El servidor SMTP utilizado para enviar correos electrónicos de notificación. Déjelo en blanco para desactivar las notificaciones por correo electrónico.
** *Puerto SMTP* - El puerto utilizado para conectarse al servidor SMTP. Si se deja vacío, se utiliza el valor predeterminado 25.
** *Nombre de usuario SMTP* - Nombre de usuario opcional para la autenticación SMTP.
** *Contraseña SMTP* - Contraseña opcional para la autenticación SMTP. Déjela en blanco para mantener la contraseña actual.
** *Habilitar SMTP STARTTLS* - Habilita STARTTLS para una comunicación segura con el servidor SMTP.
...
* *Rol* - El rol que tendrá el usuario con respecto a esta instalación del IPT.
** *Administrador* - Puede realizar cambios en todos los aspectos de la instancia IPT.
** *Administrador sin derechos de registro*: puede crear, editar, eliminar y gestionar recursos que haya creado o para cuya gestión haya sido invitado dentro de la instancia IPT. Cabe señalar que los usuarios con este rol no pueden registrar recursos ante GBIF. Además, si un recurso ya cuenta con un DOI reservado o registrado, o ya está registrado en GBIF, no pueden publicarlo.
** *Administrador con derechos de registro* – Cuenta con la capacidad adicional de registrar recursos en la red de GBIF.
** *Otros usuarios* - Pueden iniciar sesión en el IPT y ver recursos, pero no pueden realizar cambios.
