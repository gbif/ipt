...
+
--
NOTE: A metafile is *required* when an archive includes any extension files or if a single core data file uses non-standard column names in the first (header) row of data. A complete reference guide to this metafile is xref:gbif-metadata-profilx[available].
...
The IPT can be used to publish resource metadata, occurrence data, checklist data, and sampling-event data. The guide xref:how-to-publish[How to publish biodiversity data through GBIF.org] provides a simple set of instructions how to do so.

The IPT outputs a DwC-A during publishing and supports automatic registration in the GBIF network. See the xref:123-manage-resources#_visibility[IPT User Manual] for further details.
...
. xref:checklist-data#_templates[Checklist data template]: suitable for basic species checklists
. xref:occurrence-data#_templates[Occurrence data template]: suitable for occurrence data (specimen, observation)
. xref:sampling-event-data#_templates[Sampling-event data template]: suitable for sampling-event data
...
. Create a metadata file (eml.xml) that describes the data resource. Complete instructions on doing this are available in the xref:gbif-metadata-profile[GBIF Extended Metadata Profile: How-To Guide]. It is best practice to include a metadata file and the simplest way to produce one is using the IPT's built-in metadata editor.
...
. Checklist data: xref:checklist-data#_required_dwc_fields[required terms] / xref:checklist_data#_recommended_dwc_fields[recommended terms]
. Occurrence data: xref:occurrence-data#_required_dwc_fields[required terms] / xref:occurrence-data#_recommended_dwc_fields[recommended terms]
. Sampling-event data: xref:sampling-event-data#_required_dwc_fields[required terms] / xref:sampling-eventdata#_recommended_dwc_fields[recommended terms]
. Resource metadata: xref:resource-metadata#_required_dwc_fields[required terms] / xref:resource-metadata#_recommended_dwc_fields[recommended terms]
...
The guide xref:how-to-publish[How to publish biodiversity data through GBIF.org] provided a set of example DwC-As for each type of data:

. Checklist data: xref:checklist-data#_exemplar_datasets[examplar datasets]
. Occurrence data: xref:occurrence-data#_exemplar_datasets[examplar datasets]
. Sampling-event data: xref:sampling-event-data#_exemplar_datasets[examplar datasets]
. Resource metadata: xref:resource-metadata#_exemplar_datasets[examplar datasets]
