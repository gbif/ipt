...
Ideally, the publishing organization shoul not be changed after the resource has been either registered with GBIF or assigned a DOI.

That being said, there are two ways to change the publising organizations.
The easiest way to republish the resource and then reset the desired publishing organization. To simplify the process, you can recreate the dataset from its zipped IPT resource folder. Instructions on how to do that can be found xref:manage-resources.adoc#upload-a-zipped-ipt-resource-configuration-folder[here].
If you migrate the resource from the old publishing organization to the new publishing organization by following xref:manage-resources.adoc#migrate-a-resource[these instructions]. Please pay careful attention to step 5, where you will have to ask the GBIF Helpdesk to update the GBIF Registry.
...
* In <organisation>, replace the current UUID by the UUID of the organization you would like to be the new publising organization,
* Restart TomCat,
