...
To move the IPT to a different server, just make sure that you copy the entire contents of the IPT's data directory over to the new server. It is important to preserve the same folder/file permissions. Then, all you have to do is follow the xref:getting-started.adoc#starting-over[Starting Over] instructions. As it says: "If the user enters the same absolute path to the data directory as before, the previous configuration will be completely restored".
...
The account holding organization must already be associated to the IPT as an organization. For help associating an organization to your IPT, you can refer to the xref:administration.adoc#add-organisation[Add Organization] section of the User Manual.

All that remains, is to add the DataCite account information to the organization (e.g. the username, password, prefix, etc) and make sure the "Account activated" checklist has been checked. For help editing the organization and a description of each field in the form, you can refer to the xref:administration.adoc#edit-organisation[Edit Organization] section of the User Manual.
...
* Move your IPT data directory to another partition where there is more space available. Note, this requires you to xref:getting-started.adoc#starting-over[reinstall] your IPT.
...
* use the IPT's import by xref:manage-resources.adoc#taxonomic-coverage[Taxon list feature] instead of supplying each taxa manually one at a time when entering the taxonomic coverage.
...
First, ensure the desired hosting organisation has been added to your IPT. You can do this through the user interface. For help adding a new organisation to your IPT, refer to xref:administration.adoc#add-organisation[this section] of the user manual.
