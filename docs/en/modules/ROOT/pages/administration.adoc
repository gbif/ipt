...
image::ipt2/administration/IPTAdminConfigIPT1-1.png[]
image::ipt2/administration/IPTAdminConfigIPT1-2.png[]
image::ipt2/administration/IPTAdminConfigIPT1-3.png[]
...
--
* *Email notifications* - Configure SMTP settings to enable email notifications from the IPT. These notifications are used to inform the IPT administrator if auto-publication fails.
+
The following SMTP settings are available:

** *SMTP host* - The SMTP server used to send notification emails. Leave empty to disable email notifications.
** *SMTP port* - The port used to connect to the SMTP server. Defaults to 25 if left empty.
** *SMTP username* - Optional username for SMTP authentication.
** *SMTP password* - Optional password for SMTP authentication. Leave blank to keep the current password.
** *Enable SMTP STARTTLS* - Enables STARTTLS for secure communication with the SMTP server.
...
* *Plausible Analytics script URL* - If you would like to track the use of your instance of the IPT with Plausible Analytics, you can enable it to do so by entering your Plausible Analytics script URL in this text box. For more information about Plausible Analytics, see https://plausible.io/.
* *Plausible Analytics endpoint URL* (optional) - If you would like to proxy Plausible Analytics API, enter the API endpoint URL here to override the default one. For more information, see https://plausible.io/docs/proxy/introduction.
...
* *Role* - The role the user will have with respect to this installation of the IPT.
** *Admin* - Can make changes to all aspects of the instance of the IPT.
** *Manager without registration rights* - Able to create, edit, remove, and manage resources they have created or have been invited to manage within the IPT instance. Note, users with this role cannot register resources with GBIF. Moreover, if a resource already has a reserved or registered DOI, or is already registered with GBIF, they cannot publish it.
** *Manager with registration rights* - Has the additional capability to register resources with the GBIF network.
** *Other users* - Can log in to the IPT and view resources, but cannot make any changes.
