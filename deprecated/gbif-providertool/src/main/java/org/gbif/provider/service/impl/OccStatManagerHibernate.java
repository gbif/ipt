...
public class OccStatManagerHibernate extends
  ...
  public void updateRegionAndTaxonStats(OccurrenceResource resource) {
    try {
      ...
      log.debug(String.format(
          "Updating %s Region occurrence counts for resource %s",
          counts.size(), resource.getId()));
      ...
      e.printStackTrace();
      log.error("Unable to update region and taxon stats for resource "
          + resource.getId() + ": " + e.toString());
    }
