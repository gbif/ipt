 ...
 * 
 */
public class ChecklistResourceManagerHibernate extends
    DataResourceManagerHibernate<ChecklistResource> implements
    ChecklistResourceManager {
  ...
  public List<StatsCount> taxByRank(Long resourceId) {
    ...
    taxBySth = getSession().createQuery(hql).setParameter("resourceId",
        resourceId).list();
    return StatsUtils.getDataMap(taxBySth);
  }

  public String taxByRankPieUrl(List<StatsCount> data, int width, int height,
      boolean title) {
  ...
  public String taxByRankPieUrl(Long resourceId, int width, int height,
      boolean title) {
  ...
  public List<StatsCount> taxByStatus(Long resourceId, StatusType type) {
    ...
    String hql = String.format(
        "select t.%s, count(t)  from Taxon t  where t.resource.id=:resourceId and t.lft=t.rgt-1  group by t.%s",
  ...
  public String taxByStatusPieUrl(List<StatsCount> data, StatusType type,
  ...
  public String taxByStatusPieUrl(Long resourceId, StatusType type, int width,
  ...
  public List<StatsCount> taxByTaxon(Long resourceId, Rank rank) {
    ...
    if (rank == null || rank.equals(Rank.TerminalTaxon)) {
      ...
      taxBySth = getSession().createQuery(hql).setParameter("resourceId",
          resourceId).setParameter("rank", Rank.TerminalTaxon).list();
    } else {
  ...
  public String taxByTaxonPieUrl(List<StatsCount> data, Rank rank, int width,
      int height, boolean title) {
    assert (rank != null);
    String titleText = null;
    if (title) {
  ...
  public String taxByTaxonPieUrl(Long resourceId, Rank rank, int width,
      int height, boolean title) {
  ...
  public File writeTcsArchive(Long resourceId) throws IOException {
    ...
    Writer out = new BufferedWriter(new OutputStreamWriter(
        new FileOutputStream(tcs, false), "UTF8"));
    // taxa=List<Taxon>
