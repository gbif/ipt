...
public class ManagerInterceptor implements Interceptor {
  ...
  protected void handleNotAuthorized(HttpServletRequest request, HttpServletResponse response) throws ServletException,
      IOException {
  ...
  private boolean isResourceOwner(HttpServletRequest request) {
    ...
    if (requestedId != null && requestedId.getClass().isArray() && ((Object[]) requestedId).length == 1) {
